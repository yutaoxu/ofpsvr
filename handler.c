...
{
        char *brand_new;
        if (asprintf(&brand_new, "%s%s", (*old) ? (*old) : "", data ? data : "")
            < 0) {
                return 0;
        }
        if (*old)
...
{                               //-1:fail / 0:NULL / 1:SUC
        ...
        while (1) {
                ...
                if ((ptr2 = strchr(ptr1, '&'))) {
                        *ptr2 = '\0';
                        if (asprintf(&ret, "%s%s&amp;", old ? old : "", ptr1) <
                            0)
                                return -1;
                        ptr1 = ptr2 + 1;
                } else {
        ...
        while (1) {
                ...
                if ((ptr2 = strchr(ptr1, '\n'))) {
                        *ptr2 = '\0';
                        if (asprintf(&ret, "%s%s<br />", old ? old : "", ptr1) <
                            0)
                                return -1;
                        ptr1 = ptr2 + 1;
                } else {
...
struct connection_info_struct {
        ...
        long posted_at;
        enum { BAD_FORMAT, NAME_TOO_LONG, EMAIL_TOO_LONG, WEBSITE_TOO_LONG,
                BODY_TOO_LONG, BLANK
        } error_type;
        int finished;
};
                               ...
                               uint64_t off, size_t size)
{
        ...
        if (0 == strcmp(key, "reply[name]")) {
                ...
                if (sanitize(&con_info->name) < 0)
                        return MHD_NO;
                if (sanitize(&con_info->email) < 0)
                        return MHD_NO;
                if (sanitize(&con_info->website) < 0)
                        return MHD_NO;
                ...
                if (!
                    (name_escape =
                     malloc((strlen(con_info->name) * 2 + 10) * sizeof(char))))
                        return MHD_NO;
                if (mysql_real_escape_string
                    (&mm, name_escape, con_info->name,
                     strlen(con_info->name)) <= 0)
                        return MHD_NO;
                if (!
                    (email_escape =
                     malloc((strlen(con_info->email) * 2 + 10) * sizeof(char))))
                        return MHD_NO;
                if (mysql_real_escape_string
                    (&mm, email_escape, con_info->email,
                     strlen(con_info->email)) <= 0)
                        return MHD_NO;
                if (con_info->website) {
                        if (!
                            (website_escape =
                             malloc((strlen(con_info->website) * 2 +
                                     10) * sizeof(char))))
                                return MHD_NO;
                        if (mysql_real_escape_string
                            (&mm, website_escape, con_info->website,
                             strlen(con_info->website)) <= 0)
                                return MHD_NO;
                } else {
                        website_escape = 0;
                }
                if (!
                    (body_escape =
                     malloc((strlen(con_info->body) * 2 + 10) * sizeof(char))))
                        return MHD_NO;
                if (mysql_real_escape_string
                    (&mm, body_escape, con_info->body,
                     strlen(con_info->body)) <= 0)
                        return MHD_NO;

                char *sql;
                if (asprintf
                    (&sql,
                     "INSERT INTO ofpsvr_comments(`id` ,`article_id` ,`posted_at` ,`name` ,`email` ,`website` ,`body`) VALUES"
                     "(NULL,   '%d'     ,   '%ld'    , '%s'  , '%s'   , '%s'     , '%s')",
                     con_info->parsed_article_id, con_info->posted_at,
                     name_escape, email_escape,
                     (con_info->website) ? website_escape : "",
                     body_escape) < 0)
                        return MHD_NO;
                assert(name_escape);
                free(name_escape);
                ...
                case 1:
                        if ('\0' == u[1])       // "/"
                        {
                                ...
                                if (extension && (0 == strcmp("rss", extension)
                                        ...
                                        if (!
                                            (response =
                                             generate_blog_response_rss())) {
                                                return OFPSVR_Q_500;
                                        }
                                } else {
                                        if (!
                                            (response =
                                             generate_blog_response())) {
                                                return OFPSVR_Q_500;
                                        }
                                }
                                int ret;
                        ...
                        } else if ('r' == u[1]) // "/resources/..."
                        {
                                ...
                                for (resource_ptr = resources; resource_ptr;
                                     resource_ptr = resource_ptr->next)
                                        if (0 ==
                                            strcmp(resource_ptr->filename, ptr))
                                                break;
                                if (resource_ptr)
                                        return MHD_queue_response(connection,
                                                                  MHD_HTTP_OK,
                ...
                case 4:        // "/blog/.../resources/..."
                        ...
                        *ptr3 = '\0';
                        ...
                        for (resource_ptr =
                             ...
                             resource_ptr; resource_ptr = resource_ptr->next)
        ...
        } else                  // POST
        {
                ...
                case 2:        // "/blog/..."
                        if (NULL == *con_cls) {
                                ...
                                if (!
                                    (con_info->comment =
                                     calloc(1, sizeof(struct Comment)))) {
                                        free(con_info);
                                        return OFPSVR_Q_500;
                                }
                        ...
                        if (*upload_data_size) {
                                ...
                                *upload_data_size = 0;
                                ...
                                if (asprintf
                                    (&infopage,
                                     "<html><head><meta http-equiv=\"refresh\" content=\"0;URL=/blog/%d\" /><meta http-equiv=\"Content-type\" content=\"text/html; charset=UTF-8\" /></head><body>评论已发表 谢谢!</body></html>",
                                     con_info->parsed_article_id) < 0) {
                                        WRITELOG
                                            ("asprintf failed when saying comment successfully published.\n");
                                        return OFPSVR_Q_500;
                                }
                                ...
                                case NAME_TOO_LONG:
                                        return send_page(connection,
                                                         "Name too long! (Please keep its length less than 100)");
                                case EMAIL_TOO_LONG:
                                        return send_page(connection,
                                                         "Email too long! (Please keep its length less than 100)");
                                case WEBSITE_TOO_LONG:
                                        return send_page(connection,
                                                         "Website too long! (Please keep its length less than 100)");
                                case BODY_TOO_LONG:
                                        return send_page(connection,
                                                         "Body too long! (Please keep its length less than 1000)");
                                case BLANK:
                                        return send_page(connection,
                                                         "Name,Email,Body are all required fields, please don't leave any of them blank.");
                                }
                                return OFPSVR_Q_500;
                        }
                        break;
